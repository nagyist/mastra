...
The OLAP queries that back the Studio dashboards (metric aggregate, breakdown, time series, percentiles, plus the score and feedback equivalents) are implemented with standard Postgres aggregates (`SUM`, `AVG`, `percentile_cont`, `FILTER (WHERE ...)`) and a portable epoch-floor bucket expression. The same query plan works on vanilla Postgres and on a TimescaleDB hypertable.
